...
func init() {
	...
	_ = dsl.AddMultiSignatureHelperFunction("b64_utf16_enc", []string{
		"strTo string"}, func(args ...interface{}) (interface{}, error) {

			uni := unicode.UTF16(unicode.LittleEndian, unicode.IgnoreBOM)
   			encoded, _ := uni.NewEncoder().String(types.ToString(args[0]))
	...
	_ = dsl.AddMultiSignatureHelperFunction("url_path_escape_enc", []string{
		"strTo string"}, func(args ...interface{}) (interface{}, error) {

   			encoded := url.PathEscape(types.ToString(args[0]))

		return types.ToString(encoded), nil
		})
